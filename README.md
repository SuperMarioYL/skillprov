...
skillprov 把缺失的那个名词补上：**能力清单（capability manifest）**——一份可签名、声明式的「这个 skill 被允许做什么」，再配上一份它「实际包含什么」的 SBOM。验签时不只是核对签名，还会静态重扫这个 skill，把**实际观察到的能力**和**声明的能力**做差集：只要它伸手去够一个没声明的网络出站或越界写文件，就直接拒绝。
